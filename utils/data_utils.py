...
def representation_trans(s_data, source_type, target_type, dataset=None):
    ...
    if source_type in ['skeleton','dirvec','dirvec_norm']:
        # to euler angle (rotation angle)
        # s_data = np.array([[dirvec2euler(d_one, connect[-1]) for d_one, connect in zip(d_t, connects)] for d_t in s_data])
        # to euler angle (rotation angle) speed up version
        s_data = np.transpose(s_data, (1,2,0)) # N_joint, 3, Time
    ...
    # initialize_from_euler = functools.partial(R.from_euler, seq='xyz', degrees=True)
    # s_data = [R.from_euler('xyz', d_one, degrees=True) for d_one in s_data]
    s_data = R.from_euler('xyz', s_data.reshape(T*N_joint,-1), degrees=True)

    if target_type == 'quat':
        # return np.array([d_one.as_quat() for d_one in s_data]).reshape(T, N_joint, -1)
        return s_data.as_quat().reshape(T, N_joint, -1)
    elif target_type == 'rotvec':
        # return np.array([d_one.as_rotvec() for d_one in s_data]).reshape(T, N_joint, -1)
        return s_data.as_rotvec().reshape(T, N_joint, -1)
    elif target_type == 'matrix':
        # return np.array([d_one.as_matrix() for d_one in s_data]).reshape(T, N_joint, -1)
